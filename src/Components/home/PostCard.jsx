...
import  {postComments}  from '../../assets/data';
  ...
  const ReplyCard = ({ reply,  handleLike, user}) => {
            ...
            <div className='flex gap-3 items-center mb-1'>
              <Link to={"/profile/"+ reply?.userId?._id}>
                 <img className='w-10 h-10 rounded-full object-cover'
                    src={reply?.userId?.profileUrl ?? ProfilePic}
              ...
              <div>
                ...
                <span className='text-ascent-2 text-sm'>
                   {moment(reply?.createdAt ?? "2023-08-04").fromNow()}
                </span>
              </div>
    ...
    return(
      <form className='w-full border-b border-[#66666645]'
        onSubmit={handleSubmit(onSubmit)} >
         ...
         <div className='w-full flexx items-center gap-2 py-4'>
              <img className='w-10 h-10 rounded object-cover'
                  src={user?.profileUrl ?? ProfilePic }
                  alt='user image'
              />

             <TextInput name="comment" placeholder={replyAt ? `Reply @${replyAt}`: "Comment this post"}
                 register={register('comment',
                  {
                    required: 'Comment on this'
                  }
                )}
...
const PostCard = ({post, user, deletePost, likePost}) => {
    ...
    <div className='mb-2 bg-primary p-4 rounded-xl'>
       ...
       <div className='mt-4 flex justify-between items-center px-3 py-2 text-ascent-1 text-base border-t border-[#66666645]'>
              ...
              <p className='flex gap-2 items-center text-base text-ascent-1 cursor-pointer'
                       onClick={() => {
...

       {
         ...
         <div className='w-full mt-4 border-t border-[#66666645] pt-4'>
             <CommentForm
                 user={user} id={post?._id} getComments={()=> getComments(post?._id)}
                    ...
                    comments?.map((comment)=>(
                      <div className='w-full py-2' key={comment?._id}>
                        <div className='flex gap-3 items-center mb-1 '>
                          <Link>
                             <img className='w-10 h-10 rounded-full object-cover'
                                 src= {comment?.userId?.profileUrl ?? ProfilePic}
                          ...
                          <span className='text-ascent-2 text-sm'>
                              {moment(comment?.createdAt ?? "2023-12-17").fromNow()}
                          </span>
                        </div>
